node {
    ...
    withCredentials([sshUserPrivateKey(credentialsId: 'jenkins', keyFileVariable: 'identity', passphraseVariable: 'pass', usernameVariable: 'user')]){
        ...
        stage("Copy results from remote to local"){
            sshGet remote: remote, from: 'Documents/FuegoGroup/cps/test_scenarios/remoteTests/solodezaldivar/output', into: 'C:/Users/Solo de Zaldivar/Desktop/JenkinsPipe/JenkinsPipelineOutput', override:true
        }
    }
}
